...
test('THEME/LANGUAGE: storage writes must not escape as unhandled errors', () => {
  const themeMatch = source.match(/function setTheme\(value\)\{([^}]*)\}/);
  const languageMatch = source.match(/function setLanguage\(value\)\{([^}]*)\}/);
  ...
  assert.match(languageMatch[1], /try\s*\{[\s\S]*localStorage\.setItem\(LANGUAGE_STORAGE[\s\S]*\}[\s\S]*\}[\s\S]*catch/, 'language localStorage write must be guarded');
});
