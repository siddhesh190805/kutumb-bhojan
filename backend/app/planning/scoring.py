...
def score_candidate(
    ...
    if recipe_contains_ingredient(recipe, "paneer"):
        ...
        yesterday_str = datetime.fromordinal(dt.toordinal() - 1).strftime("%Y-%m-%d")
    ...
    for key, prev_rec in recent_meals_by_date_slot.items():
        prev_date = key.split("-")[0] + "-" + key.split("-")[1] + "-" + key.split("-")[2]
        day_diff = abs(dt.toordinal() - datetime.strptime(prev_date, "%Y-%m-%d").toordinal())
        ...
        if day_diff == 1 and slot == key.split("-")[-1]:
    ...
    if not is_weekend and time_min > 35:
        score -= 15.0
