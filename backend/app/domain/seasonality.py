...
Authoritative seasonal mapping for the canonical 40 ingredients grounded in
ICMR-NIN traditional food knowledge for Western India / Maharashtra household context.
...
class IngredientSeasonality(BaseModel):
    ...
    provenance: str = "icmr_nin_traditional_knowledge"
...
def get_ingredient_seasonality_entries(ingredient_key: str) -> list[IngredientSeasonality]:
    ...
    raw_list = CANONICAL_SEASONALITY_CATALOG.get(ingredient_key, [])
    results = []
    for item in raw_list:
        results.append(
            IngredientSeasonality(
                ...
                provenance="icmr_nin_traditional_knowledge",
...
def evaluate_ingredient_season(
    ...
    """
    entries = CANONICAL_SEASONALITY_CATALOG.get(ingredient_key, [])
    for e in entries:
